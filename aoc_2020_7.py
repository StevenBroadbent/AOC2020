...
rules = [rule.translate(remove_digits) for rule in data.split('\n')]
rules = [rule for rule in rules if not rule.endswith("contains no other bags.")]


def clean_up(s):
    return s.replace("bag", '').replace("bags", '').strip().replace('.', '')


can_contain = defaultdict(list)
for rule in rules:
    holding_bag, allowed_contents = map(clean_up, rule.split("s contain"))
    can_contain[holding_bag] = allowed_contents

is_contained_in = defaultdict(dict)
for rule in rules:
    parent, child_list = map(clean_up, rule.split("s contain"))
    for child in [c for c in child_list.split(',') if c.find("no other") == -1]:
        is_contained_in[child] = parent

target_bag = "shiny gold"

containing_bags = [k for (k, v) in can_contain.items() if v.find(target_bag) != -1]

for bag in containing_bags:
    containing_bags.extend([k for (k, v) in can_contain.items() if v.find(bag) != -1])

print("There are {} bags that can contain a {} bag".format(len(set(containing_bags)), target_bag))
